...
Depois que eu aprovar: ajuste a descrição da Story e crie as subtasks no board (via gh CLI), cada uma com responsável (assignee). Pra criar e conectar as subtasks (criação + vínculo de sub-issue via gh/GraphQL), siga @docs/process/operar-board.md. Não comece a implementar.
```
